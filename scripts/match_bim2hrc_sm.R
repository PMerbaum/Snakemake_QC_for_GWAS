...
# read in data
ref = read.table("/hpc/hers_en/references/HRC/HRC.r1-1.GRCh37.wgs.mac5.sites.tab.gz", header=F)
colnames(ref)[1:12] = c("chr","bp", 'ID', 'REF', 'ALT', 'AC', 'AN', 'AF', 'AC_EXCLUDING_1000G', 'AN_EXCLUDING_1000G',' AF_EXCLUDING_1000G','AA')
...
fwrite(swap, snakemake@output[[4]],col.names=F, row.names=F, quote=F, sep="\t" )